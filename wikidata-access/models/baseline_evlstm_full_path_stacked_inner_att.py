...
def train_model(data, topic, PROCESSED_DIR, SEED_FOLDER, **kwargs):
    def func(x):
        ...
        for i in range(sent_len):
            temp = TimeDistributed(paths_lstm_1)(x[:, i, :, :, :])  # [bs, max_paths, max_path_len, emb_dim] * sent_len
            temp = TimeDistributed(paths_lstm_2)(temp)  # [bs, max_paths, max_path_len, emb_dim] * sent_len
            temp = TimeDistributed(paths_lstm_last)(temp)  # [bs, max_paths, max_path_len, emb_dim] * sent_len
            liste.append(temp)
    ...
    paths_lstm_1 = LSTM(lstm_size, return_sequences=True) # define lstm that reduces the paths to one vector
    paths_lstm_2 = LSTM(lstm_size, return_sequences=True) # define lstm that reduces the paths to one vector
    paths_lstm_last = LSTM(lstm_size) # define lstm that reduces the paths to one vector

    reduce_paths_to_vector = Lambda(func, output_shape=(sent_len, max_paths, lstm_size))(emb_knowledge_ids)
