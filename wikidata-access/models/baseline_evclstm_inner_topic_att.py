...
from keras.layers import Embedding, Dropout, LSTM, Dense, Lambda, TimeDistributed
...
from models.layers.CLSTM import custom_LSTM_fo, Bidirectional
from helpers.classification.generate_features import get_avg_embedding_for_topic_list
from helpers.classification.utils import topic_vector_dim
...
def train_model(data, topic, PROCESSED_DIR, SEED_FOLDER, **kwargs):
    ...
    # TODO Note: Uses Bidirectional layer of Keras 2.1.1, since there are some exception comin up otherwise
    sent_bilstm = Bidirectional(custom_LSTM_fo(lstm_size))([concat_sequences, topic_vector_input])
