...
from models.layers.attention_tf import attention_topic_and_knowledge
import os
import random as rn
from helpers.classification.generate_features import get_avg_embedding_for_topics
...
def train_model(data, topic, PROCESSED_DIR, SEED_FOLDER, **kwargs):
    ...
    concat_sequences = tf.concat([embedded_word_ids, attended_knowledge], axis=2)  # the dimension where I want to add two tensors can be different, all others have to be equal
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(epochs): # iterate over epochs
            ...
            if monitor == "val_loss":
                ...
                temp_score = f1_score(val_y_non_one_hot, pred_list_ukp, average='macro')

            if (monitor == "val_loss" and temp_score < best_score) or (monitor == "f1_macro" and temp_score > best_score):
            ...
            print("train_loss= " + "{:.4f}".format(loss) + ", train_acc= " + \
                  ...
                  "{:.3f}".format(acc_val/(periods_ukp_val+1)) + (", val_F1= " + "{:.3f}".format(temp_score) if monitor == "f1_macro" else "")
                  )
        ...
        return sess.run(correct_pred_ukp, feed_dict={X: X_test, KX: kX_test, X_topic: X_topic_test, Y: y_test,
                                                     EMB_SENTS: emb_sents, EMB_KNOWLEDGE: emb_knowledge, dropout_const: 1.0}).tolist(),\
               sess.run(correct_pred_ukp, feed_dict={X: X_dev, KX: kX_dev, X_topic: X_topic_dev, Y: y_dev,
                                                     EMB_SENTS: emb_sents, EMB_KNOWLEDGE: emb_knowledge, dropout_const: 1.0}).tolist()