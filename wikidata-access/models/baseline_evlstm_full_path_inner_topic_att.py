...
def train_model(data, topic, PROCESSED_DIR, SEED_FOLDER, **kwargs):
    def func(x):
        ...
        for i in range(sent_len):
            liste.append(
                TimeDistributed(paths_bilstm)(x[:, i, :, :, :]))  # [bs, max_paths, max_path_len, emb_dim] * sent_len
    ...
    reduce_paths_to_vector = Lambda(func, output_shape=(sent_len, max_paths, 2*lstm_size))(emb_knowledge_ids)

    attended_knowledge = attention_knowledge(embedded_word_ids, topic_vector_input, attention_size, return_alphas=False, summed_up=True)(reduce_paths_to_vector)
