...
"""
Model that only uses the knowledge of a knowledge base. The knowledge has to be apssed sentence-wise.
"""
...
def train_model(data, topic, PROCESSED_DIR, SEED_FOLDER, **kwargs):
    ...
    sentence_inputs = Input(shape=(max_sent_len, ), dtype='int32', name="sentence_inputs")
    ...
    embedded_word_ids = Embedding(emb_knowledge.shape[0], emb_knowledge.shape[1], mask_zero=True,
                               ...
                               input_length=max_sent_len)(sentence_inputs) # [samples, sent_len, we_dim]
    ...
    e = ModelCheckpoint(model_file, monitor=monitor, verbose=0, save_best_only=True, save_weights_only=True,
                        mode='auto', period=1)
    model.fit(kX_train, y_train, batch_size=batch_size, epochs=epochs,
              validation_data=(kX_dev, y_dev), callbacks=[e], verbose=1)
    model.load_weights(model_file)

    y_pred_test = model.predict(kX_test, verbose=False)
    y_pred_dev = model.predict(kX_dev, verbose=False)
