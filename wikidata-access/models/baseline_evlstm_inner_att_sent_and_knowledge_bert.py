...
def train_model(data, topic, PROCESSED_DIR, SEED_FOLDER, **kwargs):
    ...
    bert_emb_size = kwargs['model_settings'].get('bert_emb_size', 768)
    ...
    attention_size = kwargs['model_settings'].get('attention_size', kwargs['model_settings']['word_embeddings'][1])
    ...
    sentence_inputs = Input(shape=(X_train.shape[1], bert_emb_size,), dtype='float32', name="sentence_inputs")
    knowledge_inputs = Input(shape=(sent_len, max_concepts, bert_emb_size), dtype='float32', name="knowledge_inputs")

    attended_knowledge = attention_knowledge(sentence_inputs, None, attention_size, return_alphas=False, summed_up=True)(knowledge_inputs)
